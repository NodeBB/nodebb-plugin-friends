...
Routes.initV3Routes = async function ({ router, middleware, helpers }) {
	...
	router.get('/friends/:uid', middleware.admin.checkPrivileges, middleware.assert.user, async (req, res) => {
	...
	router.post('/friends/:uid', middleware.assert.user, async (req, res) => {
		const uid = parseInt(req.user.uid, 10);
		const toUid = parseInt(req.params.uid, 10);

		if (uid === toUid) {
			helpers.formatApiResponse(400, res, new Error('[[plugin-friends:error.self-request]]'));
			return;
		}
	...
	router.delete('/friends/:uid', middleware.assert.user, async (req, res) => {
		const uid = parseInt(req.user.uid, 10);
		const toUid = parseInt(req.params.uid, 10);
...
Routes.initV1V2Routes = async function (data) {
	...
	data.router.post('/friends/:uid', data.apiMiddleware.requireUser, middleware.verifyUserExists, async (req, res) => {
		...
		if (uid === toUid) {
			data.errorHandler.handle(new Error('[[plugin-friends:error.self-request]]'), res);
			return;
		}
