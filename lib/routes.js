...
(function(routes) {
	...
	routes.initWriteRoutes = function(data, callback) {
		data.router.get('/friends/:uid', data.apiMiddleware.requireUser, data.apiMiddleware.requireAdmin, function(req, res) {
			friends.getFriendsPageData(res.locals.uid, req.params.uid, 0, 49, function(err, friendsData) {
				return data.errorHandler.handle(err, res, friendsData);
			});
		});

		data.router.get('/friends/:uid/count', function(req, res) {
		...
		data.router.post('/friends/:uid', data.apiMiddleware.requireUser, function(req, res) {
		...
		data.router.delete('/friends/:uid', data.apiMiddleware.requireUser, function(req, res) {
			var uid = parseInt(req.user.uid, 10),
				toUid = parseInt(req.params.uid, 10);
